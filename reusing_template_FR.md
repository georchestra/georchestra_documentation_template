...
- Les scripts clonent la dernière version du modèle, créent `docs/`, copient `mkdocs.yml`, `mkdocs_requirements.txt`, `readthedocs_EN.md`, `readthedocs_FR.md`, `mkdocs_installation.sh` et `mkdocs_run.sh`, puis installent les dépendances MkDocs et lancent `mkdocs serve`.
